...
class Model(object):
    ...
    def __init__(self, policy, ob_space, ac_space, nenvs, nsteps, nstack, num_procs,
        ...
        nadv = (ADV - train_model.ret_rms.mean) / (train_model.ret_rms.std + eps)
        nr = (R - train_model.ret_rms.mean) / (train_model.ret_rms.std + eps)
        ...
        def train(obs, states, returns, masks, actions, values):
            ...
            if hasattr(train_model, "ret_rms"): train_model.ret_rms.update(sess, returns) # # update running mean/std for returns
